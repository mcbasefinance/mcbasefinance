...
import fries from "../../assets/images/fries1.svg";
import cola from "../../assets/images/cola.svg";

function DollarGeyser() {
    ...
    async function fetchAllData(firstFlag = false) {
      ...
      setAllowAmount(
        await checkAllowance(
          mcbaseBusdLPTokenContract.contract,
          address,
          mcbaseDollarGeyserContract.address
        )
      );

      setTokenBalnace(await getBalance(mcbaseBusdLPTokenContract.contract, address));
      setMyTotalStaked(await totalStakedFor(mcbaseDollarGeyserContract, address));
      setRewardAmount(await getUserReward(mcbaseDollarGeyserContract, address, currentTime));
      setRewardAmountIn30Days(
        await getUserReward(mcbaseDollarGeyserContract, address, currentTime + 2592000)
      );

      const mcbaseBusdLpPrice = await getLpPrice(mcbaseBusdLPTokenContract, 'MCBASE', true, bnbPrice, mcbasePrice)
      let totalStakedAmount = await totalStaked(mcbaseDollarGeyserContract);
      setTotalTVL(totalStakedAmount.times(mcbaseBusdLpPrice));
      setTotalSecUnlockPriceInUSD(
        await getTotalSecUnlockPriceInUSD(mcbaseDollarGeyserContract, mcbasePrice, currentTime)
      );
      ...
      let firstStakeTime = await getUserFirstStakeTime(mcbaseDollarGeyserContract, address);
      if (firstStakeTime > 0) {
  ...
  const onStake = async (event) => {
    ...
    const encodedABI = mcbaseDollarGeyserContract.contract.methods
  ...
  const onApprove = async (event) => {
    ...
    const maxAmount = new BigNumber(1)
      ...
      .minus(1);
    const encodedABI = mcbaseBusdLPTokenContract.contract.methods
      .approve(mcbaseDollarGeyserContract.address, maxAmount.toString(10))
      .encodeABI();

    await sendTransaction(
      address,
      mcbaseBusdLPTokenContract.address,
      encodedABI,
      "0x61A80",
      transactionDone,
      transactionError
  ...
  return (
    <Page1>
      <PageHeader
        title="Earn More MCBASE Through The Dollar Menu Geyser"
        content=""
      />
        ...
        <Row className="justify-content-center">
          <Col xs={12} className="text-white">
            <p>
              ...
              <strong>
                <a
                  href="https://exchange.pancakeswap.finance/#/add/0xe9e7cea3dedca5984780bafc599bd69add087d56/0xcd6de21c28b352e1f7fa2966ac24edca68115159"
                  target="_blank"
                  rel="noreferrer"
              ...
              tokens into the Dollar Menu geyser to <br />
              earn more MCBASE with juicy reward mulitpliers{" "}
              <a
                href="https://mcbase-finance.medium.com/introducing-the-mcbase-dollar-menu-geyser-b09bf3f270a8"
                target="_blank"
                rel="noreferrer"
                style={{ textDecoration: "underline", color: "white" }}
              >
          ...
          <Col xs={12}>
            <Form1 title="Dollar Menu" subtitle="Geyser">
              <img className="leftSideImage3" src={fries} />
                  ...
                  <Row>
                    ...
                    <Col xs={6}>
                      <span className="numberSpan">
                        {tokenBalance.toFormat(2)} MCBASE-BUSD
                      </span>
                    </Col>
                    <Col xs={6}>
                    ...
                    <Col xs={6}>
                      <span className="numberSpan">
                        {myTotalStakedAmount.toFormat(2)} MCBASE-BUSD
                      </span>
                    </Col>
                    <Col xs={6}>
              ...
              <img className="rightSideImage3" src={cola} />
            </Form1>
          </Col>
        </Row>
      ) : (
