...
import franchise from "../../assets/images/franchise.svg";
...
function Sharebox() {
    ...
    async function fetchAllData(firstFlag = false) {
      ...
      setAllowAmount(
        await checkAllowance(
          ...
          address,
          mcbaseFranchiseGeyserContract.address
        )
      );

      setTokenBalnace(await getBalance(mcbaseBnbLPTokenContract.contract, address));
      setMyTotalStaked(await totalStakedFor(mcbaseFranchiseGeyserContract, address));
      setRewardAmount(await getUserReward(mcbaseFranchiseGeyserContract, address, currentTime));
      setRewardAmountIn30Days(
        await getUserReward(mcbaseFranchiseGeyserContract, address, currentTime + 2592000)
      );

      const mcbaseBnbLpPrice = await getLpPrice(mcbaseBnbLPTokenContract, 'WBNB', true, bnbPrice, mcbasePrice)
      let totalStakedAmount = await totalStaked(mcbaseFranchiseGeyserContract);
      setTotalTVL(totalStakedAmount.times(mcbaseBnbLpPrice));
      setTotalSecUnlockPriceInUSD(
        await getTotalSecUnlockPriceInUSD(mcbaseFranchiseGeyserContract, mcbasePrice, currentTime)
      );
      ...
      let firstStakeTime = await getUserFirstStakeTime(mcbaseFranchiseGeyserContract, address);
      if (firstStakeTime > 0) {
  ...
  const onStake = async (event) => {
    ...
    const encodedABI = mcbaseFranchiseGeyserContract.contract.methods
  ...
  const onApprove = async (event) => {
    ...
    const encodedABI = mcbaseBnbLPTokenContract.contract.methods
      .approve(mcbaseFranchiseGeyserContract.address, maxAmount.toString(10))
      .encodeABI();
  ...
  return (
    <Page1>
      <PageHeader
        title="Earn wBNB Through The McBase Franchise Geyser"
        content=""
      />
        ...
        <Row className="justify-content-center">
          <Col xs={12} className="text-white">
            <p>
              ...
              tokens into the Franchise geyser to
              earn wBNB from vault fees.{" "}
              <a
          ...
          <Col xs={12}>
            <Form1 title="McBase" subtitle="Franchise">
              <img className="leftSideImage5" src={franchise} />
              <Row className="justify-content-center align-items-center">
                ...
                <Col xs={12} lg={7}>
                  <Row>
                    ...
                    <Col xs={6}>
                      <span className="numberSpan">
                        {rewardAmountIn30Days.toFormat(4)} wBNB in 30 Days
                      </span>
                    </Col>
                    <Col xs={6}>
                    ...
                    <Col xs={6}>
                      <strong>Total Franchise TVL:</strong>
                    </Col>
                    <Col xs={6}>
                      <span className="numberSpan">
                        ${totalTVL.toFormat(2)}
