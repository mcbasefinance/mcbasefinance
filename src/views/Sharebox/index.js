...
import sharebox from "../../assets/images/sharebox.svg";
import chicken from "../../assets/images/chicken.svg";

function ShareboxGeyser() {
    ...
    async function fetchAllData(firstFlag = false) {
      ...
      setAllowAmount(
        await checkAllowance(
          mcbaseBnbLPTokenContract.contract,
          address,
          mcbaseShareboxGeyserContract.address
        )
      );

      setTokenBalnace(await getBalance(mcbaseBnbLPTokenContract.contract, address));
      setMyTotalStaked(await totalStakedFor(mcbaseShareboxGeyserContract, address));
      setRewardAmount(await getUserReward(mcbaseShareboxGeyserContract, address, currentTime));
      setRewardAmountIn30Days(
        await getUserReward(mcbaseShareboxGeyserContract, address, currentTime + 2592000)
      );

      const mcbaseBnbLpPrice = await getLpPrice(mcbaseBnbLPTokenContract, 'WBNB', true, bnbPrice, mcbasePrice)
      let totalStakedAmount = await totalStaked(mcbaseShareboxGeyserContract);
      setTotalTVL(totalStakedAmount.times(mcbaseBnbLpPrice));
      setTotalSecUnlockPriceInUSD(
        await getTotalSecUnlockPriceInUSD(mcbaseShareboxGeyserContract, mcbasePrice, currentTime)
      );
      ...
      let firstStakeTime = await getUserFirstStakeTime(mcbaseShareboxGeyserContract, address);
      if (firstStakeTime > 0) {
  ...
  const onStake = async (event) => {
    ...
    const encodedABI = mcbaseShareboxGeyserContract.contract.methods
  ...
  const onApprove = async (event) => {
    ...
    const maxAmount = new BigNumber(1)
      ...
      .minus(1);
    const encodedABI = mcbaseBnbLPTokenContract.contract.methods
      .approve(mcbaseShareboxGeyserContract.address, maxAmount.toString(10))
      .encodeABI();

    await sendTransaction(
      address,
      mcbaseBnbLPTokenContract.address,
      encodedABI,
      "0x61A80",
      transactionDone,
      transactionError
  ...
  return (
    <Page1>
      <PageHeader
        title="Earn More MCBASE Through The ShareBox Geyser"
        content=""
      />
        ...
        <Row className="justify-content-center">
          <Col xs={12} className="text-white">
            <p>
              ...
              <strong>
                <a
                  href="https://exchange.pancakeswap.finance/#/add/eth/0xcd6de21c28b352e1f7fa2966ac24edca68115159"
                  target="_blank"
                  rel="noreferrer"
              ...
              tokens into the ShareBox geyser to <br />
              earn more MCBASE with juicy reward mulitpliers{" "}
              <a
                href="https://mcbase-finance.medium.com/the-mcbase-sharebox-opens-for-business-9cd2f91034a2"
                target="_blank"
                rel="noreferrer"
                style={{ textDecoration: "underline", color: "white" }}
              >
          ...
          <Col xs={12}>
            <Form1 title="ShareBox" subtitle="Geyser">
              <img className="leftSideImage2" src={sharebox} />
                  ...
                  <Row>
                    ...
                    <Col xs={6}>
                      <span className="numberSpan">
                        {tokenBalance.toFormat(2)} MCBASE-BNB
                      </span>
                    </Col>
                    <Col xs={6}>
                    ...
                    <Col xs={6}>
                      <span className="numberSpan">
                        {myTotalStakedAmount.toFormat(2)} MCBASE-BNB
                      </span>
                    </Col>
                    <Col xs={6}>
                    ...
                    <Col xs={6}>
                      <strong>Total Sharebox TVL:</strong>
                    </Col>
                    <Col xs={6}>
                      <span className="numberSpan">
                        ${totalTVL.toFormat(2)}
              ...
              <img className="rightSideImage2" src={chicken} />
            </Form1>
          </Col>
        </Row>
      ) : (
